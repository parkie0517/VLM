...
class VisionEncoder(nn.Module):
    def __init__(self, model, hidden_size):
        ...
        model.model.fc = nn.Identity() # Remove classification head
        self.model = model # Modified ResNet18
    
        
        # Add a new linear layer to map features to hidden_size
    ...
    def forward(self, x):
        # x.shape is torch.Size([16, 3, 32, 32]) where 16 is the batch size.
        features = self.model(x)  # Extract features [batch_size, 512]

        output = self.fc(features)  # Map to desired hidden_size [batch_size, hidden_size]
        return output.unsqueeze(1)  # Reshape to [batch_size, 1, hidden_size]
...
class VisionLanguageModel(nn.Module):
    ...
    def forward(self, images, input_ids, attention_mask):
        ...
        # Update the attention_mask to match the new sequence length
        vision_mask = torch.ones((attention_mask.size(0), 1), dtype=attention_mask.dtype, device=attention_mask.device)
        updated_attention_mask = torch.cat([vision_mask, attention_mask], dim=1)

        # Pass updated embeddings and attention mask to the decoder
        outputs = self.text_decoder(inputs_embeds=embeddings, attention_mask=updated_attention_mask)
...
def train_gpt2(text_decoder, dataloader, optimizer, criterion, device, epochs=1):
    text_decoder.train()
...
def train_vlm(model, tokenizer, dataloader, optimizer, criterion, scheduler, device, epoch):
    ...
    for sample in dataloader:
        ...
        # Align logits and labels with the prepended visual token
        vision_mask = torch.full((labels.size(0), 1), -100, dtype=labels.dtype, device=labels.device)
        updated_labels = torch.cat([vision_mask, labels], dim=1)  # Add -100 for vision token

        logits = outputs.logits  # Keep logits intact; no dropping tokens

        # Flatten logits and labels for CrossEntropyLoss
        loss = criterion(logits.view(-1, logits.size(-1)), updated_labels.view(-1))
...
def save_logits(model, tokenizer, data_loader, device, LOGITS_FILE):
    ...
    with torch.no_grad():
        for batch in data_loader:
            ...
            outputs = model(images, input_ids, attention_mask)
            logits = outputs.logits[:, 1:33, :50257]  # Trim extra token and limit vocab size
...
def main():
    ...
    model_save_path = "text_decoder_model.pth"
    """
    torch.save({
        'model_state_dict': text_decoder.state_dict(),
    ...
    text_decoder.load_state_dict(checkpoint['model_state_dict'])

    text_decoder.to(device)  # Ensure model is moved to the appropriate device
    ...
    #    param.requires_grad = True
    #optimizer_vlm = optim.AdamW(model.parameters(), lr=1e-5, weight_decay=0.01)
    optimizer_vlm = optim.AdamW(filter(lambda p: p.requires_grad, model.parameters()), lr=1e-5)
